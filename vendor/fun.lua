...
--- Documentation https://luafun.github.io/index.html
...
local iterator_mt = {
  ...
  __tostring = function(self) return "<generator>" end,
...
local unwrap = function(self) return self.gen, self.param, self.state end
...
local nil_gen = function(_param, _state) return nil end
...
local rawiter = function(obj, param, state)
  ...
  elseif type(obj) == "string" then
    if #obj == 0 then return nil_gen, nil, nil end
    return string_gen, obj, 0
  end
  error(string.format('object %s of type "%s" is not iterable', obj, type(obj)))
end

local iter = function(obj, param, state) return wrap(rawiter(obj, param, state)) end
exports.iter = iter

local method0 = function(fun)
  return function(self) return fun(self.gen, self.param, self.state) end
end

local method1 = function(fun)
  return function(self, arg1) return fun(arg1, self.gen, self.param, self.state) end
end

local method2 = function(fun)
  return function(self, arg1, arg2) return fun(arg1, arg2, self.gen, self.param, self.state) end
end

local export0 = function(fun)
  return function(gen, param, state) return fun(rawiter(gen, param, state)) end
end

local export1 = function(fun)
  return function(arg1, gen, param, state) return fun(arg1, rawiter(gen, param, state)) end
end

local export2 = function(fun)
  return function(arg1, arg2, gen, param, state) return fun(arg1, arg2, rawiter(gen, param, state)) end
end
...
local range = function(start, stop, step)
  if step == nil then
    if stop == nil then
      if start == 0 then return nil_gen, nil, nil end
      stop = start
...
local duplicate_table_gen = function(param_x, state_x) return state_x + 1, unpack(param_x) end

local duplicate_fun_gen = function(param_x, state_x) return state_x + 1, param_x(state_x) end

local duplicate_gen = function(param_x, state_x) return state_x + 1, param_x end
...
local zeros = function() return wrap(duplicate_gen, 0, 0) end
exports.zeros = zeros

local ones = function() return wrap(duplicate_gen, 1, 0) end
exports.ones = ones

local rands_gen = function(param_x, _state_x) return 0, math.random(param_x[1], param_x[2]) end

local rands_nil_gen = function(_param_x, _state_x) return 0, math.random() end

local rands = function(n, m)
  if n == nil and m == nil then return wrap(rands_nil_gen, 0, 0) end
...
local head_call = function(state, ...)
  if state == nil then error "head: iterator is empty" end
  return ...
end

local head = function(gen, param, state) return head_call(gen(param, state)) end
...
local tail = function(gen, param, state)
  ...
  if state == nil then return wrap(nil_gen, nil, nil) end
...
local take_while_gen_x = function(fun, state_x, ...)
  if state_x == nil or not fun(...) then return nil end
  return state_x, ...
end
...
local drop_while_x = function(fun, state_x, ...)
  if state_x == nil or not fun(...) then return state_x, false end
  return state_x, true, ...
end
...
local drop_while = function(fun, gen_x, param_x, state_x)
  ...
  until not cont
  if state_x == nil then return wrap(nil_gen, nil, nil) end
...
local indexes_gen = function(param, state)
  ...
  while true do
    ...
    if state_x == nil then return nil end
    i = i + 1
    if r == x then return { i, state_x }, i end
  end
end

local indexes = function(x, gen, param, state) return wrap(indexes_gen, { x, gen, param }, { 0, state }) end
...
local filter = function(fun, gen, param, state) return wrap(filter_gen, { fun, gen, param }, state) end
...
local grep = function(fun_or_regexp, gen, param, state)
  ...
  if type(fun_or_regexp) == "string" then fun = function(x) return string.find(x, fun_or_regexp) ~= nil end end
...
local partition = function(fun, gen, param, state)
  local neg_fun = function(...) return not fun(...) end
...
local foldl_call = function(fun, start, state, ...)
  if state == nil then return nil, start end
...
local length = function(gen, param, state)
  if gen == ipairs_gen or gen == string_gen then return #param end
  local len = 0
  repeat
    state = gen(param, state)
    len = len + 1
...
local is_null = function(gen, param, state) return gen(param, deepcopy(state)) == nil end
...
local is_prefix_of = function(iter_x, iter_y)
  ...
  for i = 1, 10, 1 do
    ...
    if state_x == nil then return true end
    if state_y == nil or r_x ~= r_y then return false end
  end
end
...
local min = function(gen, param, state)
  ...
  if state == nil then error "min: iterator is empty" end
...
local min_by = function(cmp, gen_x, param_x, state_x)
  ...
  if state_x == nil then error "min: iterator is empty" end
...
local max = function(gen_x, param_x, state_x)
  local state_x, m = gen_x(param_x, state_x)
  if state_x == nil then error "max: iterator is empty" end
...
local map = function(fun, gen, param, state) return wrap(map_gen, { gen, param, fun }, state) end
...
local enumerate = function(gen, param, state) return wrap(enumerate_gen, { gen, param }, { 1, state }) end
...
local intersperse = function(x, gen, param, state) return wrap(intersperse_gen, { x, gen, param }, { 0, state }) end
...
local function zip_gen_r(param, state, state_new, ...)
  if #state_new == #param / 2 then return state_new, ... end
...
local zip_gen = function(param, state) return zip_gen_r(param, state, {}) end
...
local zip = function(...)
  local n = numargs(...)
  if n == 0 then return wrap(nil_gen, nil, nil) end
  local param = { [2 * n] = 0 }
  local state = { [n] = 0 }
...
local cycle = function(gen, param, state) return wrap(cycle_gen, { gen, param, state }, deepcopy(state)) end
